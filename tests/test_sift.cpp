...

    {
        for (int method = 2; method < 3; ++method) { // тестируем три метода: OpenCV ORB, OpenCV SIFT, ваш SIFT
            ...
            if (method == 0) {
                ...
                // DONE remove 'return' and uncomment
//                return;
                method_name = "SIFT_MY";
                log_prefix = "[SIFT_MY] ";
                phg::SIFT mySIFT;
            ...
            if (angle_diff_sum != 0.0) {
                ...
                // DONE почему SIFT менее точно угадывает средний угол отклонения? изменяется ли ситуация если выкрутить параметр ORIENTATION_VOTES_PEAK_RATIO=0.999? почему?
            }
